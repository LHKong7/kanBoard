...
| Q5 | **可控出境**：允许包含 `confidential` 的数据发送至外部托管模型（见 [ADR-0006](../adr/0006-model-data-egress.md)）；`pii` 字段仍默认脱敏；出境范围由租户级开关控制且全量审计 |
...
| FR-AI-014 | 数据出境控制：默认允许至已批准的外部模型供应商，`pii` 脱敏，全量审计 | M | 未批准的供应商端点被拒绝；出境记录含租户/对象/字段分级；租户开关可一键收紧 |
