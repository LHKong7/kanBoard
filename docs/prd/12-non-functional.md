...
| NFR-PERF-003 | 图遍历：深度 ≤ 5、**结果 ≤ 500 节点**、并发 ≤ vCPU 数 | P95 < 500ms |
| NFR-PERF-003b | 同上，并发 = 2× vCPU 数 | P95 < 800ms |
...
>
> **已于 2026-08-03 在 100 万节点样本上标定**，结果见 [性能基线](../perf-baseline.md)。
>
> NFR-PERF-003 的原文是「深度 ≤ 5，1000 万节点」——它规定了数据集大小，
> 却没有规定**扇出**与**并发**。而深度 5 可以触及 5 个节点也可以触及一千万个，
> 同一条目标下，链式遍历与全子树展开是两个数量级的工作量。这样的目标无法证伪。
> 现改写为约束结果规模与并发的形式，并拆出 003b 覆盖过载情形。
>
> ⚠️ 1000 万节点规模仍未验证，只测到 100 万（M0 出口标准要求的样本）。
