...
导入时我把 ADR 正文截断到 20 000 字符、标题截断到 1024、知识正文到 200 000。
**这三个数字全是我编的。**

我当初把这条记成"没有长度上限"。**这是错的**，而错得很有意思：
上限一直都在，只是长在**校验器**里而不是本体里——

| kind | 当时的实际上限 | 谁看得到 |
| --- | --- | --- |
| `string` | 1,024 | 只有服务端 |
| `text` | 1,000,000 | 只有服务端 |
| `richtext` | 1,000,000（与 text 共用） | 只有服务端 |
| `json` | 无 | —— |

所以真正的缺陷不是"没有约束"，是**约束不可发现**。
客户端无从知道多长会被拒，只能各自猜一个数去截断；猜得不一样，
同一份内容在不同客户端就以不同的方式被截断。写导入脚本时我就是这么干的。

- 修复：`maxLength` 上到 `AttributeType`，按 `kind` 取默认值，
  **在注册时补成具体数字**，随 `GET /v1/ontology/entity-types` 一起发出。
  校验器、接口、照本体渲染表单的前端读的是同一个数——
  默认规则要是在三处各实现一遍，迟早有一处漂移，
  而漂移的表现是"这里能存那里存不下"，又一种没人报错的错。
- 数值照真实语料定（自用库 180 行）：

  | kind | 实测最长 | 定为 |
  | --- | --- | --- |
  | `richtext` | 7,468（Knowledge.body） | 200,000 |
  | `text` | 1,465（Decision.chosen） | 20,000 |
  | `string` | 79（Requirement.title） | 1,024（沿用） |

- **`text` 与 `richtext` 不再共用一个数**：一个「阻塞原因」能写一兆字节
  不是用法，是缺陷。`json` 也第一次有了上限（按序列化长度算）。
- 导入脚本改成从本体读上限，那三个编出来的数字删掉了。
- UI 上输入框的 `maxlength` 来自本体，正文类字段还会把上限写在提示里——
  否则用户只能一直写下去、提交时才被拒，而在那之前他并不知道边界在哪。

##### 顺带定下的一条规则

收紧约束到底算不算破坏性变更，**取决于现有数据有没有越界**——
同一个改动在空库上无害，在生产库上可能是灾难。这件事可以查，那就不该猜。

`tools/check-ontology-fit.ts` 用当前本体校验库里每一行，有越界就非零退出。
1.2.0 就是这么定的：180 行全部通过，因此按 minor 处理。
规则写进了 [04-ontology 的兼容矩阵](prd/04-ontology.md#6-本体版本与演进)。
...
| #6 | 接受一个不完整的图 | 自己把边补上 |
| #6b | 接受指向虚空的引用 | 422 |
| #5 | 让读只能用 POST | 给视图一个 URL |
| #6 | 接受一个不完整的图 | 自己把边补上 |
| #6b | 接受指向虚空的引用 | 422 |
| #6c | 返回一个不存在的 id | 返回真实存在的那条 |
| #7 | 把约束藏在校验器里 | 随类型定义一起公布 |
...
**一、一条记录背后往往不止一个缺陷。**
#6、#6b、#6c 是同一次深挖挖出来的。最初记的是"导入脚本漏了 Knowledge 的边"，
看起来像脚本的疏忽；往下追才发现是同一个事实被存了两份而没有不变式，
再往下又撞见外键根本不存在、以及重复建边会返回假 id。
**照着记录去挖，比照着记录去修更值。**

**二、记录本身也可能记错，动手时要重新查。**
#7 我记的是"没有长度上限"。真去看才发现上限一直都在，
只是长在校验器里而不是本体里——真正的缺陷是**不可发现**，不是不存在。
两种毛病的修法完全不同：前者要加约束，后者要把已有的约束搬到看得见的地方。
按最初那句话去做，只会再加一层重复的校验，而客户端还是得猜。
**摩擦的感受是可靠的，对成因的判断不是。**
...
**这一轮的 9 条全部处理完了。** 下一次自用应当从零开始重新记录——
这份日志的价值在于它记的是**真实使用中被绊到的地方**，
而不是一份可以照着打勾的待办清单。
