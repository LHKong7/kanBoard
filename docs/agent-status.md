...
| FR-AGT-013 | 模型分级路由，供应商可插拔 | `src/infrastructure/model/pi.ts`，底座是 pi（[ADR-0013](adr/0013-pi-as-model-substrate.md)）。档位→模型的路由在 `ModelPolicy` 里，改它即换模型 |
...
| FR-AGT-014 | 成本归因到业务对象 | Run 上有 `costUsd`（现在是供应商真实计价），未按 Project/Requirement 汇总 |
| FR-AGT-015 | 采纳率统计 | 依赖 Dashboard |
| FR-AGT-016 | 租户自建 Agent | Agent 已经是普通资源，缺模板与配额 |

## 关于模型客户端

`ModelClient` 是端口，现在有两个实现：

| `PROJECTOS_MODEL` | 实现 | 用在哪 |
| --- | --- | --- |
| `none`（缺省） | 不启动 runner | 没配模型的部署 |
| `scripted` | `ScriptedModelClient`，确定性回复、不出网 | 验证链路本身是不是通的 |
| `pi` | `PiModelClient`，经 [pi](https://github.com/earendil-works/pi) 接真实供应商 | 真正干活 |

pi 接在端口**下面**，不是替换 `AgentRuntime`——理由与边界见
[ADR-0013](adr/0013-pi-as-model-substrate.md)。一句话：
**底座是模型接入，不是 Agent 语义**。上面那张"已交付"的表里每一条
都长在运行时里，换底座之后仍然逐条被用例锁着
（`tests/integration/pi-runtime.test.ts` 走真数据库、真 RLS、真 PDP，
只有模型那一格是写死的）。

确定性实现留着，而且仍然是测试的主力。这不是"假装有 AI"，
而是两件事的分离：
...
`PROJECTOS_MODEL=pi` 守的是同一条：凭据缺失、模型名写错、路由指向
未批准的供应商——三种情况都在**进程启动时**抛，而不是等到某条 Run
半夜跑起来才暴露，也不会降级成一个还能返回点什么的实现。
