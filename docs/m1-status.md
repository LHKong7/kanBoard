...
| **3** | **没有搜索** | **记不住 id 的人无法使用系统**——自用时只能把 142 条需求全量拉到内存用正则找。UI 上"关系目标只列最近 50 条"是同一个缺失的表现 | **M1 内**（自用后从 M2 提前，见[自用日志 #3](dogfooding-log.md)） |
| 4 | 图可以不完整而系统不出声 | 本体只声明关系的 domain/range，无法声明"必须被连上"；导入时漏建的 16 条 Knowledge 边无人提醒 | M1，[自用日志 #6](dogfooding-log.md) |
| 5 | 读路径只有 `POST :query`，没有 GET 投影 | 看板视图没有 URL：分享不了、收藏不了、前进后退不工作 | 需设计决策，[自用日志 #5](dogfooding-log.md) |
| 6 | 文本属性没有长度上限 | `AttributeType` 无长度约束，客户端只能各自猜一个数 | M1，[自用日志 #7](dogfooding-log.md) |
...
第 3–6 项来自第一次真实自用，完整记录见[自用日志](dogfooding-log.md)。
同一次迭代发现并**已修复**的还有 4 项：接口静默吞掉写错的字段、
