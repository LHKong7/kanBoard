...
| **收紧约束**（新增/降低 `maxLength`、缩小枚举取值） | **取决于数据** | 先查现有数据是否越界：无越界则 minor，有越界则 major + 迁移 |
...
- 破坏性变更必须有 ADR。
- **收紧约束前必须查，不能猜。** 同一个改动在空库上无害、在生产库上是破坏性的，
  差别只在数据。`tools/check-ontology-fit.ts` 用当前本体校验库里每一行，
  有越界就非零退出：
  ...
  1.2.0（长度上限进本体）就是这么定的：180 行全部通过，因此按 minor 处理。

## 6.1 属性长度上限

每个文本类属性都带一个**可被客户端读到**的 `maxLength`，
随 `GET /v1/ontology/entity-types` 一起发出。不填则按 `kind` 取默认值：

| kind | 默认上限 | 语义 |
| --- | --- | --- |
| `string` | 1,024 | 短标签：标题、名字、principal |
| `text` | 20,000 | 一段话 |
| `richtext` | 200,000 | 一篇文档 |
| `json` | 100,000 | 按序列化后的长度算 |

这条之所以必须长在本体上而不是留在校验器里：**上限是语义的一部分**。
藏在服务端时它照样在执行，只是客户端无从知道边界在哪，
只能各自猜一个数去截断——猜得不一样，同一份内容在不同客户端
就以不同的方式被截断（[自用日志 #7](../dogfooding-log.md)）。

数值取自真实语料而非拍脑袋，`kind` 本来就承载着
「短标签 / 一段话 / 一篇文档」的语义，上限跟着它走即可，
个别属性需要另设时在该属性上写 `maxLength` 覆盖。
