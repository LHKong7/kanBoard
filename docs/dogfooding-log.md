...
**唯独漏了 Knowledge**——16 条知识现在不属于任何项目。

系统一声没吭。本体声明了关系的 domain/range（谁能连谁），
但没有任何办法声明**义务**（谁必须被连上）。
`Knowledge` 的生命周期里有「发布需要来源关系」这种守卫，
可它停在 `Draft`，守卫就永远不会被触发。

这不是导入脚本的 bug——脚本按我写的做了。是系统**接受了一个不完整的图**
而没有任何提示。要么本体支持必需关系（`minCardinality`），
要么有一个「孤儿对象」视图能把它们捞出来。目前两者都没有。
...
值得注意的是这 7 处里没有一处是「功能没做完」——
状态机、权限、租户隔离、图查询都按设计工作了。
挡路的全部是**说不出话**：写错了不报错、没推进不给原因、找不到东西没有入口、
图不完整没有提示。
...
1. **搜索**（#3）——记不住 id 的人现在无法使用这个系统
2. **孤儿与必需关系**（#6）——图默默地不完整，比图是空的更危险
